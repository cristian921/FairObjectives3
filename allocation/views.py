...
from .forms import *
from allocation.utilities.utility import *
...
@login_required(login_url='/login/')
def delete_resources(request, pk, template_name='allocation/confirm_delete.html'):
    resources = get_object_or_404(UserResource, pk=pk)
    if request.method == 'POST':
        resources.delete()
...
def index(request):
    if ObjectiveSolution.objects.filter(objective__user=request.user):
        return HttpResponseRedirect('/viewObjectivesAllocation/')
    else:
        ...
        portfolio = AggregatedPortfolio.objects.filter(user=request.user)
        if portfolio:
            asset_portfolio = PortfolioAsset.objects.filter(aggregated_portfolio__in=portfolio).order_by()
            prices = Series.objects.filter(asset__portfolioasset__aggregated_portfolio__in=portfolio, date=today)
            initial_prices = Series.objects.filter(asset__portfolioasset__aggregated_portfolio__in=portfolio,
                                                   date__lte=portfolio.first().start_date).order_by('-date')
            asset_series = Series.objects.filter(asset__portfolioasset__aggregated_portfolio__in=portfolio,
                                                 date__range=(portfolio.first().start_date, today))
            asset_series_dict = dict()
            print(asset_series)
            for ap in asset_portfolio:
                asset_series_dict[ap.asset.codec] = pd.DataFrame(list(asset_series.filter(asset=ap.asset).values()))
...
def objectives_allocation_fun(request):
    ...
    if portfolio:
        ...
        for obKey in objectives:
            ...
            in_prices = Series.objects.filter(asset__portfolioasset__aggregated_portfolio__in=portfolio,
...
def delete_solution(request, pk):
    ...
    if request.method == 'POST':
        obs.delete()
        return HttpResponseRedirect('/view_objective_allocation/')
